...
#+BEGIN_SRC emacs-lisp
  ...
  (defun emacsmate-package-dir (package)
    ...
    (let ((pkg-vec (cdr (assq package package-alist)))
      ...
      (when pkg-vec
        (when (fboundp 'package-desc-vers)
          (setq available-version (package-version-join (package-desc-vers pkg-vec))))
        (when (fboundp 'package-desc-version)
          (setq available-version (package-version-join (package-desc-version pkg-vec))))
...
#+begin_src emacs-lisp
  ...
  (when (fboundp 'rainbow-mode)
    (mapc
     (lambda(x)
       (add-hook x 'rainbow-mode))
     '(emacs-lisp-mode-hook R-mode-hook)))
#+end_src
